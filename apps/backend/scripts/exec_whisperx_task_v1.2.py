...
try:
    ...
    if log_files:
        ...
        # Extract date part from latest log file name
        latest_date_match = re.search(r"sparrow-(\d{4}-\d{2}-\d{2})\.plog", log_files[0])
        if not latest_date_match or latest_date_match.group(1) != today_str:
            # Create a new log file for today if date differs
            latest_log_file = os.path.join(log_path, f"sparrow-{today_str}.plog")
    else:
        # No log files found — create today's log
        latest_log_file = os.path.join(log_path, f"sparrow-{today_str}.plog")
...
try:
    ...
    COMPUTE_TYPE = "int8" if DEVICE == "cpu" else config.get("compute_type", "float16")
...
try:
    ...
    model = whisperx.load_model(MODEL_SIZE, DEVICE, compute_type=COMPUTE_TYPE, threads=1)
except Exception as e:
...
# ---------------------------
try:
    if language_preferred == 0:
        language_code = result["language"]
    elif language_preferred == 1:
        language_code = "zh"
    elif language_preferred == 2:
        language_code = "en"
    else:
        language_code = result["language"]  
    ...
    logger.info(f"Align whisperx output using language {language_code} (preferred={language_preferred}) and model: WAV2VEC2_ASR_LARGE_LV60K_960H")
    model_a, metadata = whisperx.load_align_model(language_code=language_code, device=DEVICE, model_name="WAV2VEC2_ASR_LARGE_LV60K_960H")
...
logger.info(f"Detected Language: {align_result['language']}")
all_subtitles = []

# 根據 CKIP 斷詞結果，重組 segment['words'] 為新的片段串列。
# 每個新片段的 start = 第一個 word 的 start，end = 最後一個 word 的 end
def regroup_words_by_ckip(segment, ckip_words):
    whisper_words = segment.get('words', [])
    n = len(whisper_words)
    start = segment.get('start', 0)
    end = segment.get('end', start)
    duration = end - start

    # 如果沒有 whisper_words，線性分配
    if not whisper_words:
        total = len(ckip_words)
        ...
        return [
            ...
            for i, word in enumerate(ckip_words)
        ]

    new_segments = []
    idx = 0
    prev_end = start
    for ckip_word in ckip_words:
        combined_word = ''
        words_to_combine = []
        start_idx = idx

        # 合併 whisper_words 直到組成 ckip_word
        while idx < n and len(combined_word) < len(ckip_word):
            w = whisper_words[idx]
            # 補 None
            if w.get('start') is None or w.get('end') is None:
                w['start'] = prev_end
                w['end'] = prev_end
            combined_word += str(w['word'])
            words_to_combine.append(w)
            idx += 1

        # 計算 start/end
        starts = [w.get('start') for w in words_to_combine if w.get('start') is not None]
        ends = [w.get('end') for w in words_to_combine if w.get('end') is not None]
        # 若整批都 fallback 到 segment['start']，且這不是第一個 word
        if starts and ends and not (all(s == start for s in starts) and all(e == start for e in ends) and new_segments):
            seg_start = starts[0]
            seg_end = ends[-1]
        else:
            # fallback 用 prev_end
...
# 如果 segment['text'] 長度 > 25，則計算「第 25 個字元」是落在哪個 segment['words'] 的 word 上，
# 並用這個 word 的結尾作為 max_line_length（即：讓這一行盡量在這個 word 結束處切割，不會切開 word）。
# 如果 segment['text'] 長度 <= 25，則直接不切。
def get_word_boundary_max_length(segment, limit=25):
    text = segment['text']
    if len(text) <= limit:
        return None  # 不需要切割
    ...
    for i, w in enumerate(segment['words']):
        word_text = str(w['word'])  # 防止不是字串
        char_count += len(word_text)
        if char_count >= limit:
            # 計算從頭到這個 word（含）為止的總長度
            total = 0
            for j in range(i+1):
                total += len(str(segment['words'][j]['word']))
            return total
    # 若總長度都沒超過 limit，回傳全文長度
    return len(text)
...
try:
    ...
    #處理，如果segment['words']內容是空的，就抓segment['text']去補上，並補上線性時間
    for segment in align_result['segments']:
        ...
        if align_result['language'] == 'zh':
            # 呼叫 CKIP 斷詞（無論有無 words 都重新斷）
            words = ws_driver([segment['text']])[0]

            print(f"text: {segment['text']} LEN: {len(segment['words'])}")
            ...
            print(f"before regroup text: {segment['text']} LEN: {len(segment['words'])}")
            regrouped = regroup_words_by_ckip(segment, words)
            #print("=== regroup 後 ===")
            #for w in regrouped:
            #    print(f"{w['word']}\t{w['start']} ~ {w['end']}")
            print(f"after regroup text: {segment['text']} LEN: {len(segment['words'])}")

            segment['words'] = regrouped

            print(f"assigned segment text: {segment['text']} LEN: {len(segment['words'])}")
            #print(f"=== segment後 ===")
            #print(f"text: {segment['text']}")
            #print(f"start: {segment['start']}, end: {segment['end']}")
        ...
        #)
        if align_result['language'] == 'zh':
            if len(segment['text']) <= 25:
                # 不處理，直接傳給 SubtitlesProcessor
                processor = SubtitlesProcessor(
                    [segment],
                    lang=align_result['language'],
                    max_line_length=25,
                ...
                ml = get_word_boundary_max_length(segment, 25)
                processor = SubtitlesProcessor(
                    [segment],
                    lang=align_result['language'],
                    max_line_length=ml,
